...
class Trayecto extends CI_Controller
{
	...
	public function crearTrayectoPost()
	{
		...
		if($this->input->post("submitOk"))
		{
			...
			$this->form_validation->set_rules('horaLlegada', 'Hora llegada', 'required|trim|callback__horaRegex');
			$this->form_validation->set_rules('horaRetorno', 'Hora llegada', 'required|trim|callback__horaRegex');
			$this->form_validation->set_rules('comentarios', 'comentarios', 'max_length[140]');
            ...
            $this->form_validation->set_message('exact_length','El campo %s debe tener 5 números');
            $this->form_validation->set_message('less_than','El campo %s debe ser menor a 53000');
	...
	//SE PUEDE AGREGAR EN LIBRARIES/FORM_VALIDATION.PHP
